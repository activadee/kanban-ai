...
- CLI workspace: from `cli/`, `bun run build` compiles via `tsc`, `bun run test` runs Vitest. Releases run through `semantic-release` (`bun run release`, triggered by `.github/workflows/release.yml`), which is now manually dispatched (`workflow_dispatch`), validates the repo via `bun run test && bun run build`, then builds binaries, publishes the npm package, updates `CHANGELOG.md`, and uploads assets to GitHub Releases. PR titles must follow Conventional Commits; `.github/workflows/semantic-pr.yml` enforces the check, and the old manual release workflows (`release-binary.yml`, `release-cli.yml`) remain as fallbacks.
