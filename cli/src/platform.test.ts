...
describe("resolveBinaryInfo", () => {
    ...
    it("resolves win32 x64 binary name candidates and cache subdirs", () => {
        ...
        expect(info.assetNameCandidates).toEqual([
            "kanban-ai-win-x64.exe",
            "kanban-ai-win-x64",
        ]);
    ...
    it("detects current platform/arch as supported", () => {
        const result = detectPlatformArch();
        expect(["linux", "darwin", "win32"]).toContain(result.platform);
        expect(["x64", "arm64"]).toContain(result.arch);
    });
});
