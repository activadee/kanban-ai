...
interface MasterDetailLayoutProps<T extends MasterDetailItem = MasterDetailItem> {
    title: string
    items: T[]
    ...
    loading?: boolean
    renderItem?: (item: T, isActive: boolean, defaultRender: () => ReactNode) => ReactNode
    sidebarFooter?: ReactNode
    sidebarClassName?: string
}

export function MasterDetailLayout<T extends MasterDetailItem = MasterDetailItem>({
    title,
    items,
    activeId,
    onSelect,
    ...
    const renderDefaultItem = (item: T, isActive: boolean) => {
    ...
    return (
        <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
            <aside className={cn(
                'hidden w-60 shrink-0 border-r border-border/50 bg-background md:flex md:flex-col',
                sidebarClassName
            )}>
                    ...
                    {loading ? (
                        ...
                        <ul className="space-y-0.5">
                            {items.map((item) => {
                                ...
                                const defaultRender = () => renderDefaultItem(item, isActive)

                                return (
                                    <li key={item.id}>
                                        {renderItem
                                            ? renderItem(item, isActive, defaultRender)
                                            : defaultRender()}
                                    </li>
                                )
                            })}
