...
import {getWorktreePath, getWorktreePathByNames, createWorktree} from '../ports/worktree'
...
function deserializeConversationItem(row: { id: string; ts: Date | number; itemJson: string }): ConversationItem {
    try {
        ...
        return {type: 'error', timestamp: new Date().toISOString(), text: 'Failed to load conversation entry'}
    }
}
...
export async function getLatestAttemptForCard(boardId: string, cardId: string) {
    ...
    return {attempt: selected, logs, conversation: items.map(deserializeConversationItem)}
}

export async function startAttempt(input: StartAttemptInput, deps?: AttemptServiceDeps) {
    ...
    const base = existing?.baseBranch ?? input.baseBranch ?? settings.baseBranch ?? 'main'

    let branch = existing?.branchName ?? input.branchName
    if (!branch) {
        const tmpl = settingsService.snapshot().branchTemplate || '{prefix}/{ticketKey}-{slug}'
        ...
        branch = renderBranchName(tmpl, {prefix, ticketKey: ticket, slugSource})
    }
    if (!branch) branch = `kanbanai/${Math.random().toString(36).slice(2, 8)}`
    const defaultWorktreePath = boardRow ? getWorktreePathByNames(boardRow.name, cardRow?.title ?? `card-${input.cardId}`) : getWorktreePath(input.boardId, id)
    ...
    const previousStatus: AttemptStatus | undefined = existing?.status as AttemptStatus | undefined
    ...
    if (!existing) {
        ...
        if (existing.status === 'running' || existing.status === 'stopping') throw new Error('Attempt already running')
    ...
    queueMicrotask(async () => {
        ...
        try {
            ...
            const emit = async (
                ...
                if (isAborted && (evt.type === 'log' || evt.type === 'conversation')) return
                if (evt.type === 'log') {
                    const ts = new Date();
                    ...
                    await updateAttempt(id, {status: nextStatus, updatedAt: new Date()})
                    events.publish('attempt.status.changed', {
                        attemptId: id,
                        boardId: input.boardId,
                        status: nextStatus,
                    ...
                    try {
                        await updateAttempt(id, {updatedAt: new Date(), sessionId: evt.id})
                    } catch {
                    }
                    try {
                        const ts = new Date();
            ...
            if (profileId) { /* host can resolve profiles externally if needed; skip here to keep core generic */
            }
            const log = async (level: 'info' | 'warn' | 'error', message: string) => {
                await emit({type: 'log', level, message})
            }
            logFn = log
            const code = typeof agent.run === 'function' ? await agent.run({
                attemptId: id,
                boardId: input.boardId,
                cardId: input.cardId,
                worktreePath,
            ...
            } as any, profile as any) : 1
            await log('info', `[runner] agent exited with code ${code}`)
            const final: AttemptStatus = (running.get(id)?.aborted ? 'stopped' : code === 0 ? 'succeeded' : 'failed')
            const endedAt = new Date();
            await updateAttempt(id, {status: final, endedAt, updatedAt: endedAt})
            events.publish('attempt.status.changed', {
                attemptId: id,
                boardId: input.boardId,
                status: final,
            ...
            try {
                await updateAttempt(id, {status: 'failed', updatedAt: new Date()})
            } catch {
            }
        } finally {
            try {
...
export async function followupAttempt(attemptId: string, prompt: string, profileId?: string, deps?: AttemptServiceDeps) {
    ...
    if (!base.sessionId) throw new Error('No session recorded for this attempt');
    if (base.status === 'running' || base.status === 'stopping') throw new Error('Attempt already running')
    ...
    if (!worktreePath) {
        ...
        worktreePath = boardRow ? getWorktreePathByNames(boardRow.name, cardRow?.title ?? base.cardId) : getWorktreePath(base.boardId, base.id)
    }
    await updateAttempt(base.id, {status: 'queued', worktreePath, updatedAt: now, startedAt: null, endedAt: null})
    events.publish('attempt.queued', {
        attemptId: base.id,
        ...
        try {
            ...
            const emit = async (
                ...
                if (evt.type === 'log') {
                    ...
                    currentStatus = next;
                    await updateAttempt(base.id, {status: next, updatedAt: new Date()});
                    events.publish('attempt.status.changed', {
                        attemptId: base.id,
                        boardId: base.boardId,
                        status: next,
                    ...
                    try {
                        await updateAttempt(base.id, {updatedAt: new Date(), sessionId: evt.id})
                    } catch {
                    }
                    ;events.publish('attempt.session.recorded', {
                        attemptId: base.id,
            ...
            let profile: unknown = agent.defaultProfile;
            if (effectiveProfileId) { /* host can resolve profile JSON externally if desired */
            }
            try {
                ...
                } as any, profile as any);
                await emit({type: 'log', level: 'info', message: `[runner] agent exited with code ${code}`});
                await emit({type: 'status', status: code === 0 ? 'succeeded' : 'failed'})
            } catch (err) {
                await emit({
                    type: 'log',
                    level: 'error',
