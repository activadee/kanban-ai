...
vi.mock("fs/promises", () => ({
    mkdir: (...args: unknown[]) =>
        mkdirMock(...(args as Parameters<typeof mkdirMock>)),
}));

vi.mock("../src/fs/repos", () => ({
    discoverGitRepositories: (...args: unknown[]) =>
        discoverMock(...(args as Parameters<typeof discoverMock>)),
}));
...
describe("fs/git", () => {
    ...
    it("initializes repo when requested and path uses tilde expansion", async () => {
        ...
        expect(mkdirMock).toHaveBeenCalledWith("/home/dev/project", {
            recursive: true,
        });
    ...
    it("throws when directory is not a git repository", async () => {
        ...
        await expect(
            ensureGitRepository("/tmp/not-repo", false),
    ...
    it("retrieves origin url and trims value", async () => {
        const raw = vi
            .fn()
            .mockResolvedValue(" https://github.com/org/repo.git \n");
