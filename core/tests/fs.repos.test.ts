...
const readdirMock = vi.fn<
    Promise<Dirent[]>,
    [string, { withFileTypes: true } | undefined]
>();
const homedirMock = vi.fn<() => string | undefined>();

vi.mock("fs/promises", () => ({
    readdir: (path: string, opts?: { withFileTypes: true }) =>
        readdirMock(path, opts as any),
}));
...
describe("fs/repos", () => {
    ...
    it("respects skip list and returns sorted unique results", async () => {
        ...
        expect(visitedPaths.some((path) => path.includes("node_modules"))).toBe(
            false,
        );
    });
    ...
    it("falls back to default search roots when base path not provided", async () => {
        const cwdSpy = vi
            .spyOn(process, "cwd")
            .mockReturnValue("/home/test/work");
