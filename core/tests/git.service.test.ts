...
describe("git/service basic operations", () => {
    it("resolves repository path or throws when missing", async () => {
        ...
        await expect(getRepoPath("missing")).rejects.toThrowError(
            "Project not found",
        );
    });
...
describe("git/service repository status and content", () => {
    ...
    it("commits and pushes with appropriate arguments", async () => {
        ...
        expect(git.raw).toHaveBeenCalledWith(["push", "upstream", "main"]);
    });
    ...
    it("reads file content from different sources", async () => {
        ...
        const base = await getFileContent(
            "proj",
            "README.md",
            "base",
            "origin/main",
        ...
        const unknown = await getFileContent(
            "proj",
            "README.md",
            "headless" as any,
        );
...
describe("git/service helpers for worktree paths", () => {
    it("resolves base refs and ancestors", async () => {
        const { resolveBaseRefAtPath, resolveBaseAncestorAtPath } =
            await loadService();
        ...
        const ref = await resolveBaseRefAtPath(
            "/tmp/work",
            "develop",
            "upstream",
        );
        ...
        const ancestor = await resolveBaseAncestorAtPath(
            "/tmp/work",
            "develop",
            "upstream",
        );
    ...
    it("computes status against base and reads file variants", async () => {
        const { getStatusAgainstBaseAtPath, getFileContentAtPath } =
            await loadService();
        const git = createGitMock("/tmp/work");
        git.status.mockResolvedValue({
            current: "feature",
        ...
        const status = await getStatusAgainstBaseAtPath(
            "/tmp/work",
            "base-ref",
        );
        ...
        expect(status.files.slice().sort(sortByPath)).toEqual(
            expectedFiles.sort(sortByPath),
        );
        expect(status.summary).toMatchObject({
            added: 3,
            modified: 2,
        ...
        const worktree = await getFileContentAtPath(
            "/tmp/work",
            "a.txt",
            "worktree",
        );
        ...
        const base = await getFileContentAtPath(
            "/tmp/work",
            "a.txt",
            "base",
            "origin/main",
    ...
    it("publishes events for commit and push operations", async () => {
        ...
        await commitAtPath("/tmp/work", "Commit message", undefined, {
            projectId: "proj",
            attemptId: "att",
        });
        await pushAtPath(
            "/tmp/work",
            { setUpstream: true },
            { projectId: "proj", attemptId: "att" },
        );

        expect(bus.publish).toHaveBeenCalledWith("git.status.changed", {
            projectId: "proj",
        });
        ...
        await expect(
            mergeBranchIntoBaseForProject("proj", {
                remote: "origin",
                baseBranch: "main",
                headBranch: "feature",
            }),
