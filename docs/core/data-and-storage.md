...
- Location:
  - By default, the database file lives in the OS-specific application data directory (e.g.
    `~/.local/share/kanbanai/kanban.db` on Linux).
  - You can override the location with `DATABASE_URL`, using:
    - `sqlite:/absolute/path/to/kanban.db` or `file:/absolute/path/to/kanban.db`.
- Migrations:
