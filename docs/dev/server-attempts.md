...
1. **Start / Resume**
    ...
    - Follow-up requests now allow optional image attachments: the handler validates PNG/JPEG/WebP (max four files, 5 MB each), requires either a prompt or at least one image, and enforces a request-body size cap before parsing. Attachments are persisted to disk under `.kanbanai/attachments/<attemptId>` and conversation history stores lightweight API-base-relative URLs (`attempts/:id/attachments/<file>`) rather than full base64 data URLs, keeping DB and event-stream payloads small (the server serves these at `GET /api/v1/attempts/:id/attachments/:fileName`). Vision-capable runtimes can still receive local file paths (`ctx.images`) and agents that accept base64 (e.g., OpenCode) still receive `ctx.attachments` in-memory for that turn. The cap is configurable via `KANBANAI_MAX_FOLLOWUP_BODY_BYTES` (default 32 MB) and hitting it returns HTTP `413`.
