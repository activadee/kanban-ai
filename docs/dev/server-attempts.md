...
- Support **Planning Attempts** (`isPlanningAttempt`) alongside standard Implementation Attempts. Planning Attempts use a specialized prompt and are treated as side-effect-free (no auto-commit / no board automation).
...
1. **Start / Resume**
    ...
    - The async runner streams structured JSONL from the agent. Each message is persisted and emitted as an event:
        ...
        - Attempt lifecycle events include an `isPlanningAttempt` flag so listeners can ignore planning attempts when appropriate.
...
2. **Completion**
    - Runner emits `attempt.completed` with final status. Git auto-commit is delegated via
      ...
      Planning Attempts do not trigger auto-commit or board automation listeners.
