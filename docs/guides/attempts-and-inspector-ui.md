...
Switching cards recalculates which top-level tab should be active: cards without Attempts fall back to **Ticket**, while cards with Attempts open on **Attempts** (and its inner tabs). This recalculation also happens when attempt data changes for the current card—if a new Attempt auto-starts or loads while you still have the inspector open, it automatically flips to **Attempts** so you immediately see the running attempt’s **Messages** view. If you manually lock the view on **Ticket**, that choice stays in place until you open a different card, preventing the auto-switch from interrupting your current context. Navigating between Attempts, or when a new Attempt is detected, still resets the inner tab back to **Messages**, so you always start from the same view when focusing on new work.
