...
- **Inbox** – Actionable `review`, `failed`, and `stuck` buckets derived from `DashboardInbox` that surface reminders, failures, and stuck work in the same time range; items are sorted by recency and include relative timestamps.
