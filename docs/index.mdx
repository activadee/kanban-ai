...
- **High‑level metrics** – See total projects, active Attempts, attempts in the selected time range (default last 7 days), and open cards.
...
- **System status** – Quick status for GitHub connectivity and registered agents, including per‑agent activity and success rates over the current Dashboard time range.
- **Time range presets** – Mission Control exposes a preset selector (`Last 24 hours`, `Last 7 days`, `Last 30 days`) that scopes every metric, inbox, and agent stat; switching presets refetches the snapshot with a dedicated query key so the WebSocket stream stays live on the default range.
