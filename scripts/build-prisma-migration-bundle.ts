...
async function main() {
    ...
    if (!exists) {
        console.error(
            "[build-prisma-migration-bundle] prisma/migrations not found; cannot build migration bundle",
        );
        process.exit(1);
    }
    ...
    if (migrations.length === 0) {
        console.error(
            "[build-prisma-migration-bundle] no Prisma migrations found in prisma/migrations; refusing to generate empty bundle",
        );
        process.exit(1);
    }
