...
import {updateProjectSettingsSchema} from "./project.schemas";
...
export const getProjectSettingsHandlers = createHandlers(
...
export const previewNextTicketKeyHandlers = createHandlers(
    zValidator("param", projectIdParam),
    async (c) => {
        const {projects} = c.get("services");
        const {projectId} = c.req.valid("param");
...
export const updateProjectSettingsHandlers = createHandlers(
    zValidator("param", projectIdParam),
