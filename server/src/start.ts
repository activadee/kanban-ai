...
export async function createWebSocket(): Promise<{
  ...
  return { upgradeWebSocket: upgradeWebSocket as UpgradeWebSocket<AppEnv>, websocket }
}

async function bootstrapRuntime(migrationsDir?: string) {
  const { migrate } = await import('drizzle-orm/bun-sqlite/migrator')
  const resolvedMigrations = await resolveMigrationsFolder(migrationsDir)
  await migrate(db, { migrationsFolder: resolvedMigrations })
